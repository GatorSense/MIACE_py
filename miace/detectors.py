...
def smf_det(hsi_data, tgt_sig, mu=None, sig_inv=None, tgt_flag=False):
    ...
    Inputs:
      ...
      mu - Dx1 vector containing the background mean vector, if empty, computed as mean of all hsi_data
      sig_inv - DxD matrix containing the inverse background covariance, if empty, computed from all hsi_data
    ...
    """
    z, st_sig_inv, st_sig_inv_s, mu, sig_inv = detector_helper(hsi_data, tgt_sig, mu, sig_inv, tgt_flag)
...
def ace_det(hsi_data, tgt_sig, mu=None, sig_inv=None, tgt_flag=False):
    ...
    Outputs:
      ace_data - Nx1 vector of ACE confidence values corresponding to each test point
      mu - Dx1 vector containing the background mean vector
      sig_inv - DxD matrix containing the inverse background covariance
    """
...
def detector_helper(hsi_data, tgt_sig, mu=None, sig_inv=None, tgt_flag=False):
    ...
    sig_inv = sig_inv if sig_inv is not None else np.linalg.pinv(np.cov(hsi_data.T))
